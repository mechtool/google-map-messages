import {
  Component, ElementRef, Inject, ChangeDetectorRef, Renderer2, ViewChildren, QueryList,
...
export class SvgMessageFilterComponent implements OnChanges{
  ...
  constructor(private elementRef : ElementRef, public changeRef : ChangeDetectorRef, @Inject(DOCUMENT) private documentRef : any, private renderer : Renderer2, public gisService : GisService){
    ...
    function processSvg(){
      ...
      for(let key0 = 0; key0 < allKeys.length; key0++){
        ...
        for(let key1 = 0; key1 < 2 ; key1++){ //0 - 1
          ...
          that.leftDate = Math.min(that.leftDate, messRect.dateNumber);
          that.rightDate = Math.max(that.rightDate, messRect.dateNumber);
